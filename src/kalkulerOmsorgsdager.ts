...
export const ALENEOMSORG_KRONISK_SYKT_BARN_DAGER: number =
  KRONISK_SYKT_BARN_DAGER * 2;
...
const harOmsorg = (barn: Barn): boolean =>
  !!(barn.alder === AlderType.UNDER12 || barn.kroniskSykt);

export const grunnrettsdager = (
  barn: Barn[],
...
export const kroniskSyktDager = (
  barn: Barn[],
  inkluderKoronadager: boolean
): Omsorgsdager => {
  const kroniskOgDeltOmsorg = barn.filter(
    (b) => b.kroniskSykt && !b.søkerHarAleneomsorgFor
  );
...
export const aleneomsorgKroniskSykeDager = (
  barn: Barn[],
  inkluderKoronadager: boolean
): Omsorgsdager => {
  const kroniskOgAleneomsorg = barn.filter(
    (b) => b.kroniskSykt && b.søkerHarAleneomsorgFor
  );
  const dager =
    ALENEOMSORG_KRONISK_SYKT_BARN_DAGER * kroniskOgAleneomsorg.length;
  const koronafaktor = inkluderKoronadager ? 1 : 0;

  return { normaldager: dager, koronadager: dager * koronafaktor };
};

export const aleneomsorgsdager = (
  barn: Barn[],
  inkluderKoronadager: boolean
): Omsorgsdager => {
...
export const sumOverføringsdager = (
  foreldre: Forelder[],
  inkluderKoronadager: boolean
): Overføringsdager =>
    ...
    (tmpDager, forelder) => ({
      overførteKoronadager: inkluderKoronadager
        ? tmpDager.overførteKoronadager +
          (forelder.koronadager?.dagerTildelt || 0)
        : 0,
      mottatteKoronadager: inkluderKoronadager
        ? tmpDager.mottatteKoronadager + (forelder.koronadager?.dagerFått || 0)
        : 0,
      mottatteNormaldager:
        tmpDager.mottatteNormaldager + (forelder.normaldager?.dagerFått || 0),
      fordelteNormaldager:
        tmpDager.fordelteNormaldager +
        (forelder.normaldager?.dagerTildelt || 0),
    }),
    {
...
export const beregnOmsorgsdager = (
  barn: Barn[] = [],
  inkluderKoronadager: boolean
): Omsorgsprinsipper => {
  ...
  return {
    ...
    aleneomsorgKroniskSyke: aleneomsorgKroniskSykeDager(
      barnMinimumUtfylt,
      inkluderKoronadager
    ),
  };
